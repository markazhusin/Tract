...
Адрес узла **вводить не нужно**: клиенты находят живой узел сами (встроенные
seed-узлы + опрос `/health`). Достаточно, чтобы в сети был хотя бы один живой узел.
...
Публичный узел уже развёрнут на Railway и вшит в клиенты как seed — для простого
старта свой узел поднимать необязательно.
...
## Деплой узла на Railway

В репозитории есть `railway.json` и `Dockerfile`. После пуша в GitHub Railway сам
собирает и поднимает узел (домен с HTTPS). Узел слушает `PORT`, данные пишет в
`./data` — **подключи volume на `/app/data`**, иначе при редеплое потеряются
identity-блобы и офлайн-сообщения (сервер не хранит пароли и не может восстановить ключ).
