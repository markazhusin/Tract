...
// EnqueuePendingMessageControl stores a message_control packet for offline delivery.
// Used for delete_messages and clear_chat commands when recipient is offline.
func (s *Storage) EnqueuePendingMessageControl(toUserId string, packet map[string]interface{}) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	entry := &InboxEntry{
		Id:         generateID(),
		Type:       "app_packet",
		FromUserId: packet["senderId"].(string),
		ToUserId:   toUserId,
		Payload:    packet,
		Timestamp:  time.Now().UnixMilli(),
	}
...
// GetPendingMessageControls retrieves and clears message_control packets for a user.
func (s *Storage) GetPendingMessageControls(toUserId string) ([]map[string]interface{}, error) {
	s.mu.Lock()
	defer s.mu.Unlock()

	entries := s.inboxes[toUserId]
	pending := make([]map[string]interface{}, 0)

	for _, e := range entries {
		if e.Payload != nil {
			if pt, ok := e.Payload["type"].(string); ok && pt == "message_control" {
				pending = append(pending, e.Payload)
			}
		}
	}

	// Remove processed message_control entries
	filtered := make([]*InboxEntry, 0, len(entries))
	for _, e := range entries {
		if e.Payload != nil {
			if pt, ok := e.Payload["type"].(string); ok && pt == "message_control" {
				continue // skip, it's processed
			}
		}
		filtered = append(filtered, e)
	}
	s.inboxes[toUserId] = filtered
	s.saveJSON("message-inbox.json", s.inboxes)
